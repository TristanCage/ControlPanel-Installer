...
########################################################
# 
#               ControlPanel Installation Script
#
#               Maintained by Goodness Tech
#
...
WIKI_LINK="https://github.com/TristanCage/ControlPanel-Installer/wiki"
# UPDATED: Pointing to the main branch of the new repository
GITHUB_URL="https://raw.githubusercontent.com/TristanCage/ControlPanel-Installer/main"
...
# Fetch latest version number from the main branch of the new repo
LATEST_VERSION="$(curl -s https://raw.githubusercontent.com/TristanCage/panel/main/config/app.php | grep "'version'" | cut -c18-25 | sed "s/[',]//g")"
}
...
# FIX: Restart php-fpm to ensure it loads the newly installed extensions (like Redis)
...
# Assuming the panel is now hosted by TristanCage/panel based on the installer change
git clone -q https://github.com/TristanCage/panel.git /var/www/controlpanel
...
# Install Dependencies - FIXES: php8.1-intl and php8.1-redis added
...
# Enable services
enable_services_debian_based
# FIX: Restart PHP-FPM to load the newly installed extensions (like Redis)
...
if [ "$OS_VER_MAJOR" == "7" ]; then
    ...
    # Install dependencies - FIXES: php-intl and php-redis added
